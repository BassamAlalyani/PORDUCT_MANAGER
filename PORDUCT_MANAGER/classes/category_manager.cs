...
namespace PORDUCT_MANAGER.classes
{
    ...
    {
        DA.DataAccess da = new DA.DataAccess();//  استدعاء كلاس الاتصال  داخل كلاس ادارة الاصناف 
        public void add_category(string name)// اظافة صنف جديد
        {
            SqlParameter[] parameter = new SqlParameter[1];
            parameter[0] = new SqlParameter("@category_name ", SqlDbType.NVarChar, 50); // الاسم الاول فيه حسب الموجد في قاعدة البيانات 

            parameter[0].Value = name;
            da.open(); // دالة فتح الاتصال مع قاعدة ال..
            da.Excutecommend("add_category", parameter); // الاسم فيه حسب اسم الاجراء الموجود في قاعدة البيانات 
        ...
        public DataTable show_categories() // عرض الصنف الجديد واظهاره في القايمه
        {
            ...
            DataTable dt = new DataTable();// جدول يحمل الاعمدة والصفوف من قاعدة البيانات الى العرض 
            dt = da.SelectData("show_category", null);
            return dt;


        }
        public void edit_category(int id , string name)// زر لتعديل العنصر المحدد 
        {
